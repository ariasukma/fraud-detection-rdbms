...
Oracle supports simple and advanced connection modes.

Simple mode uses `DB_HOST`, `DB_PORT`, and `DB_NAME` as host, port, and service name:
...
Advanced mode uses `ORACLE_DSN` directly. When `ORACLE_DSN` is set, Oracle adapters ignore `DB_HOST`, `DB_PORT`, and `DB_NAME`.

Oracle Docker PDB `FREEPDB1`:
...
Oracle single instance service name:

```env
DB_TYPE=oracle
ORACLE_DSN=db01.example.com:1521/ORCLPDB1
DB_USER=fraud_user
DB_PASSWORD=fraud_password
```

Oracle RAC using SCAN:

```env
DB_TYPE=oracle
ORACLE_DSN=rac-scan.example.com:1521/FRAUDPDB
DB_USER=fraud_user
DB_PASSWORD=fraud_password
```

Oracle RAC using full connect descriptor:

```env
DB_TYPE=oracle
ORACLE_DSN=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=rac-scan.example.com)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=FRAUDPDB)))
DB_USER=fraud_user
DB_PASSWORD=fraud_password
```
